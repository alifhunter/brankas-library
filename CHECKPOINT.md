...
| 2026-06-22 | Component card thumbnails                | Added a `thumbnail` upload field to `ComponentPages`; `getComponentThumbnails()` maps canonical slug → URL and the home Components grid renders it in the card (falling back to the pink placeholder when unset). Additive `thumbnail_id` column pushed to Neon. Verified by setting/clearing a test thumbnail in prod. |
